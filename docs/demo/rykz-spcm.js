...
// EXTERNAL MODULE: ./node_modules/css-loader/dist/cjs.js!./node_modules/less-loader/dist/cjs.js!./ui/rykz-spcm.less
var rykz_spcm = __webpack_require__(5602);
;// CONCATENATED MODULE: ./ui/rykz-spcm.less
...
var update = injectStylesIntoStyleTag_default()(rykz_spcm/* default */.Z, options);




       /* harmony default export */ const ui_rykz_spcm = (rykz_spcm/* default */.Z && rykz_spcm/* default.locals */.Z.locals ? rykz_spcm/* default.locals */.Z.locals : undefined);

// EXTERNAL MODULE: ./node_modules/core-js/modules/es.object.assign.js
var es_object_assign = __webpack_require__(9601);
;// CONCATENATED MODULE: ./ui/js/constants.js
...
var TYPES_LEFT_RIGHT = ['integer', 'integerrange', 'decimal', 'decimalrange', 'formid', 'formtype', 'key', 'values', 'number', 'boolean', 'string', 'action'];
var TYPES_TOP_DOWN = ['list', 'map'];
var LINKED_ELEMENTS = ['input', 'button'];
var KEY_DXSCANCODE_MAP = {
  'Escape': 0,
  '1': 2,
  '2': 3,
  '3': 4,
...
var reverseMapping = function reverseMapping(o) {
  return Object.keys(o).reduce(function (r, k) {
    return Object.assign(r, _defineProperty({}, o[k], (r[o[k]] || []).concat(k)));
  }, {});
};

var DXSCANCODE_NAME_MAP = {
  0: 'Unbound',
  1: 'Escape',
  2: '1',
  3: '2',
...
var NAME_DXSCANCODE_MAP = reverseMapping(DXSCANCODE_NAME_MAP);
var NAME_FORMTYPE_MAP = {
  "ANIO": 83,
  "ARMA": 102,
  "AcousticSpace": 16,
  "Action": 6,
...
var pluginTemplate = function pluginTemplate(index, name, categoryList, edited, raw) {
  var categories = categoryList.map(function (categoryName, categoryIndex) {
    return "\n    <button".concat(raw ? ' raw' : '', " onclick=\"dataSelect(").concat(index, ", ").concat(categoryIndex, ")\">").concat(categoryName, "</button>\n  ");
  }).join('');
  return "\n    <div id=\"m-plugin-".concat(index, "\">\n    <button").concat(edited ? ' edited' : '').concat(raw ? ' raw' : '', " onclick=\"dataSelect(").concat(index, ", 0)\">").concat(name, "</button>\n    <div categories>\n      ").concat(categories, "\n    </div>\n    </div>\n");
};
...
var mPlugins = f('m-plugins');
var mSettings = f('m-settings');
var mSettingsTitle = f('m-settings-container').children[0];
var mOverlayKey = f('m-overlay-key');
var mSaveRevert = f('m-settings-container').children[1];
var mToggleSPCMOnly = f('m-plugins-container').children[1];
var mContextMenu = f('m-context-menu'); ////////// DATA FUNCTIONS

var data = {};
var dataKeys = [];
var categoryKeys = [];
var loadedDataPath = null;
var settingsKeys = [];
var selectedPlugin = null;
var selectedCategory = null;
var pendingChanges = false;
var filterSPCMOnly = true;
var hoveredDataPath = null;
...
function dataSet(dataURIComponent) {
  ...
  for (var i = 0; i < dataKeys.length; i++) {
    ...
    for (var j = 0; j < categoryKeys[i].length; j++) {
...
function dataRefresh() {
  ...
  for (var i = 0; i < dataKeys.length; i++) {
    var raw = data[dataKeys[i]].title == dataKeys[i];
    if (filterSPCMOnly && raw) continue;
    mPlugins.appendChild(elementFromHTML(pluginTemplate(i, data[dataKeys[i]].title, Object.entries(data[dataKeys[i]].categories).map(function (categoryKV) {
      return categoryKV[1].title;
    }), data[dataKeys[i]]['_edited'], raw)));
  }
...
function dataSelect(dataIndex, categoryIndex) {
  ...
  selectedPlugin = f("m-plugin-".concat(dataIndex));
...
function dataLoad(dataIndex, categoryIndex) {
  var categoryKeys = Object.keys(data[dataKeys[dataIndex]].categories);
  var settings = data[dataKeys[dataIndex]].categories[categoryKeys[categoryIndex]].settings;
  if (!settings) return;
  var settingsKeys = Object.keys(settings);
  mSettings.innerHTML = '';

  var _loop = function _loop(settingIndex) {
    var setting = settings[settingsKeys[settingIndex]];
    var settingElement = elementFromHTML(settingTemplate("".concat(dataIndex, ",").concat(categoryIndex, ",").concat(settingIndex), setting['name'], setting['desc'], setting['type'], setting['default'], setting['_value'], setting['_friendlyName'], setting['_edited']));
    mSettings.appendChild(settingElement);
    settingElement.addEventListener('mouseenter', function () {
      hoveredDataPath = settingElement.id.slice(10).replaceAll('-', ',');
    });
    settingElement.addEventListener('mouseleave', function () {
      hoveredDataPath = null;
    });
  };

  for (var settingIndex = 0; settingIndex < settingsKeys.length; settingIndex++) {
    _loop(settingIndex);
  }

  loadedDataPath = "".concat(dataIndex, ",").concat(categoryIndex);
...
function dataToggleFilter() {
  var button = mToggleSPCMOnly.children[0];
...
function linkStart(htmlElement) {
  var _iterator = _createForOfIteratorHelper(LINKED_ELEMENTS),
      _step;
  ...
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      ...
      var _iterator2 = _createForOfIteratorHelper(htmlElement.getElementsByTagName(TAG)),
          _step2;

      try {
        var _loop2 = function _loop2() {
          var element = _step2.value;
          var dataPath = element.getAttribute('path');
          ...
          switch (ensureType) {
            ...
            case 'key':
              element.addEventListener('mousedown', function (e) {
                ...
                var getKeyFunction = function getKeyFunction(mouseEvent) {
                  mouseEvent.preventDefault();
                  var key = mouseEvent.key;
                  if (mouseEvent.location == KeyboardEvent.DOM_KEY_LOCATION_LEFT) key = 'L' + key;else if (mouseEvent.location == KeyboardEvent.DOM_KEY_LOCATION_RIGHT) key = 'R' + key;else if (mouseEvent.location == KeyboardEvent.DOM_KEY_LOCATION_NUMPAD) key = 'NP' + key;
                  var scancode = KEY_DXSCANCODE_MAP[key] || 0;
                  if (scancode == 0) console.log("Can't bind key: ".concat(key));
                ...
                var clickDetectFunction = function clickDetectFunction() {
                  mOverlayKey.setAttribute('hidden', '');
                  document.removeEventListener('keyup', getKeyFunction);
            ...
            case 'boolean':
              element.addEventListener('mousedown', function (e) {
                e.preventDefault();
                if (e.button != 0) return;
                var value = !convertToValue(element.value, ensureType);
            ...
            case 'integerrange':
              element.addEventListener('input', function () {
                ...
                var value = convertToValue(this.value, ensureType);
            ...
            case 'decimal':
              ...
              element.addEventListener('change', function () {
                var x = this.value;
                if (!x.toString().includes('.')) x = x + '.';
                if (x.startsWith('.')) x = '0' + x;
                if (x.startsWith('-.')) x = x.replace('-.', '-0.');
                if (x.endsWith('.')) x = x + '0';
                this.value = x;
                var value = convertToValue(x, ensureType);
                dataPathSet(dataPath, '_value', value);
              });
              break;
            ...
            case 'number':
              ...
              element.addEventListener('change', function () {
                var x = this.value;
                if (x == '') x = '0';else if (x.startsWith('.')) x = '0' + x;else if (x.startsWith('-.')) x = x.replace('-.', '-0.');else if (x.endsWith('.')) x = x.slice(0, -1);
                this.value = x;
                var value = convertToValue(x, ensureType);
                dataPathSet(dataPath, '_value', value);
              });
            ...
            case 'formtype':
              ...
              element.addEventListener('change', function () {
                var x = this.value;
                if (!FORMTYPES.includes(x)) x = 'None';
                this.value = x;
                var value = convertToValue(x, ensureType);
                dataPathSet(dataPath, '_value', value);
              });
            ...
            case 'values':
              ...
              element.addEventListener('change', function () {
                var x = this.value;
                if (!ensureVariables.includes(x)) x = ensureVariables[0];
                this.value = x;
                var value = convertToValue(x, ensureType);
                dataPathSet(dataPath, '_value', value);
              });
...
function dataPathSet(dataPath, property, value) {
  var setElementValue = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
        ...
        switch (ensureType) {
          ...
          case 'map':
            ...
            if (settingElement) {
              ...
              newSettingElement.addEventListener('mouseleave', function () {
                hoveredDataPath = null;
              });
...
function dataPathElement(dataPath) {
  ...
  try {
    switch (parts.length) {
      case 1:
        // plugin
        return f("m-plugin-".concat(parts[0])).children[0];

      case 2:
        // category
        return f("m-plugin-".concat(parts[0])).children[1].children[parts[1]];
...
function dataPathMarkEdited(dataPath) {
  var edited = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
...
function saveEditedCheck() {
  for (var i = 0; i < dataKeys.length; i++) {
    dataPathMarkEdited("".concat(i), false, false);

    for (var j = 0; j < categoryKeys[i].length; j++) {
      dataPathMarkEdited("".concat(i, ",").concat(j), false, false);

      for (var k = 0; k < settingsKeys[i][j].length; k++) {
        if (data[dataKeys[i]].categories[categoryKeys[i][j]].settings[settingsKeys[i][j][k]]['_edited']) {
          dataPathMarkEdited("".concat(i), true, false);
          dataPathMarkEdited("".concat(i, ",").concat(j), true, false);
          dataPathMarkEdited("".concat(i, ",").concat(j, ",").concat(k), true, false);
        }
      }
    }
  }
...
function saveStart() {
  if (loadedDataPath) {
    var fileName = dataKeys[loadedDataPath.split(',')[0]];
    var saveData = encodeURIComponent(JSON.stringify(data[fileName]));
...
function saveRevert() {
  if (loadedDataPath) {
    var i = loadedDataPath.split(',')[0];

    for (var j = 0; j < categoryKeys[i].length; j++) {
      for (var k = 0; k < settingsKeys[i][j].length; k++) {
        var dataPath = "".concat(i, ",").concat(j, ",").concat(k);
...
function contextMenuSetup() {
  ...
  var _loop3 = function _loop3(i) {
    mContextMenu.children[i].addEventListener('click', function () {
      contextMenuAction(i);
    });
  };

  for (var i = 0; i < mContextMenu.children.length; i++) {
    _loop3(i);
  }
}
...
function s() {
  if (window.skyrimPlatform && window.skyrimPlatform.sendMessage) window.skyrimPlatform.sendMessage.apply(null, arguments);else console.log("Sending: ".concat(Array.from(arguments)));
}
...
function elementMarkEdited(htmlElement) {
  var edited = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  ...
  inputElement.addEventListener('input', function () {
    ...
    try {
      var _loop4 = function _loop4() {
        ...
        var button = elementFromHTML(_this.value != '' ? "\n        <button>\n          ".concat(result.slice(0, _this.value.length), "<span>").concat(result.slice(_this.value.length), "</span>\n        </button>\n      ") : "\n        <button>\n          ".concat(result, "\n        </button>\n      "));
